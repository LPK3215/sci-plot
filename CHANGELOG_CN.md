...
- 新增工作流架构图（`docs/architecture.svg`）及生成脚本（`docs/scripts/generate_architecture.py`）。
- 在 README.md 中添加徽章（许可证、版本、CodeBuddy Skill）。
- 在 README.md 中添加项目结构树、FAQ 章节和作者信息。
- 在 README.md 中添加可视化架构图章节。
- 新增交互式项目全景观览页（`project_overview.html` + `project_overview/`），支持深色主题、动画工作流时间线、风格预览和 HTML/CSS 架构图。
- 更新架构 SVG，新增项目全景观览区和 layout-rules.md 引用。
- 更新 README 项目结构树，新增 `project_overview/` 目录。
